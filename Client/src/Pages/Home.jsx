...
const Home = () => {
  ...
  useEffect(() => {
    ...
    if (lastSwipeTime) {
      const hoursPassed =
        (Date.now() - parseInt(lastSwipeTime)) / (1000 * 60 * 60);
      if (hoursPassed >= 12) {
  ...
  const handleSwipeLimit = () => {
    if (!user?.subscribtion && swipeCount >= 10) {
  ...
  const nextUser = async (id) => {
    ...
    try {
      await axios.post(
        `${API_URL}/api/user/rightSwipe/${id}`,
        { id },
        {
  ...
  useEffect(() => {
    const getAllProfile = async () => {
      try {
        ...
        const currentUserProfile = await axios.get(
        ...
        if (currentUserProfile.data) {
          ...
          const receiveRequest =
            currentUserProfile.data.data.receiveRequest || [];
          const matched = currentUserProfile.data.data.matched || [];
          const preference =
            currentUserProfile.data.data.profile?.preference || {};
          ...
          <div className="max-w-md w-full">
            ...
            <div
              className={`flex flex-col justify-center ${
                profiles.length === 0 ? "items-center h-full" : ""
              }`}
            >
              {profiles.length > 0 ? (
                <div
                  className={`w-full bg-white rounded-3xl shadow-xl overflow-hidden relative transition-all duration-300 ${
                    isLiked ? "animate-pulse" : ""
                  } ${isPassed ? "animate-shake" : ""}`}
                >
                  {/* Image Carousel */}
                      ...
                      <div className="flex justify-between items-end">
                        <div>
                          ...
                          <div className="flex items-center text-sm mt-1">
                            ...
                            <span>
                              {currentUser.profile?.city || "Location not set"}
                            </span>
                          </div>
                        </div>
                        <button
                  ...
                  <div className="p-4">
                    ...
                    <div className="grid grid-cols-2 gap-4 text-sm">
                      <div>
                        ...
                        <p className="text-gray-800">
                          {currentUser.profile?.zodiac || "Not set"}
                        </p>
                      </div>
                      <div>
                        <h3 className="text-xs font-semibold text-gray-500 mb-1">
