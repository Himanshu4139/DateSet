...
const UserProfile = () => {
  ...
  useEffect(() => {
    const fetchProfile = async () => {
      try {
        const res = await axios.get(`http://localhost:4000/api/user/me/${id}`, {
    ...
    <div className="min-h-screen bg-gradient-to-b from-gray-50 to-purple-50">
      ...
      <div className="container mx-auto px-4 py-8 max-w-2xl">
        <div className="bg-white rounded-2xl shadow-xl overflow-hidden transition-all duration-300 hover:shadow-2xl">
          {/* Profile Header */}
          <div className={`${gradient} pt-8 pb-6 px-6 text-center`}>
            <div className="relative inline-block group">
              ...
              />
              <div className={`absolute bottom-0 right-2 w-5 h-5 rounded-full border-2 border-white ${
                profile.accountStatus === 'active' ? 'bg-green-500' : 'bg-gray-400'
              }`}></div>
            </div>
                ...
                content={
                  <div className="flex flex-wrap gap-2">
                    {profile.profile?.interests?.map((interest) => (
                      <span key={interest} className="px-3 py-1 bg-indigo-100 text-indigo-800 rounded-full text-sm">
                        {interest}
                      </span>
          ...
          <div className="bg-gray-50 px-6 py-3 border-t border-gray-100">
            <p className="text-center text-sm text-gray-500">
              {profile.accountStatus === 'active' ? 'Active now' : 'Inactive'} • Member since {formatDate(profile.createdAt)}
            </p>
          </div>
        </div>
      </div>
