...
import { FaPaperPlane, FaUserCircle, FaDownload, FaInfoCircle } from "react-icons/fa";
...
const Chat = ({ avatar }) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        if (otherUserId) {
          const otherRes = await fetch(`${API_URL}/api/user/me/${otherUserId}`, {
          ...
          const messagesRes = await fetch(`${API_URL}/api/messages/${otherUserId}`, {
            credentials: 'include'
          });
          if (messagesRes.ok) {
  ...
  const handleFileSend = async (file) => {
    try {
      ...
      // 2. Send file to backend using axios
      await axios.post(
        `${API_URL}/api/user/premiumMatch/${otherUserId}`,
        {},
        { withCredentials: true }
  ...
  const renderFileMessage = (message) => {
    ...
    switch (message.fileType) {
      ...
      default:
        return (
          <div className={`flex items-center justify-between bg-pink-50 p-3 rounded-lg border border-pink-200 ${maxWidth}`}>
      ...
      <header className="fixed top-0 left-0 right-0 z-10 w-full max-w-3xl mx-auto flex items-center justify-between p-4 bg-white/90 backdrop-blur border-b border-pink-100 shadow-sm">
        <div className="flex items-center">
          <div className="relative">
            ...
            />
            <div className={`absolute bottom-0 right-0 w-3 h-3 rounded-full border-2 border-white ${isConnected ? 'bg-green-400' : 'bg-gray-400'
              }`}></div>
          </div>
          <div className="ml-4">
            <h1 className="text-lg font-semibold text-pink-600">{otherUser?.name}</h1>
            <p className="text-xs text-pink-400">
              {isConnected ? 'Online' : `Last seen ${formatLastSeen(otherUser?.lastActive)}`}
            </p>
          </div>
        </div>
        <button
        ...
        <div className="w-full max-w-2xl mx-auto space-y-3">
          {messages.map((message, index) => (
            <div
              ...
              className={`flex ${message.sender ? "justify-end" : "justify-start"} group`}
            >
              <div
                className={`relative max-w-xs md:max-w-md px-10 py-2 rounded-2xl shadow-md ${message.sender
              ...
              >
                ...
                <div className={`flex items-center justify-end mt-1 space-x-1 text-xs ${message.sender ? 'text-pink-100' : 'text-gray-500'
                  }`}>
                  <span>{message.time}</span>
                  {message.sender && (
                    <BsCheck2All className={message.read ? "text-white" : "text-pink-200"} />
                  )}
                </div>
              </div>
            </div>
